...
void send(char *syscallType,clientPacket packetToSend){		/*Function that performs PostOffice->Send, which takes the data from client to server*/
	...
	if(packetToSend.syscall == SC_EndSimulation){
		printf("%s : END SIMULATION CALLED BY THE MANAGER, ENDING THE SIMULATION",syscallType);
		for(i=0;i<SERVERS;i++){
			pFromCToS.to=i;											/*Setting the machine ID and the MailBox ID for the client and server*/
			mFromCToS.to=0;
			pFromCToS.from = myMachineID;
			mFromCToS.from=currentThread->myProcessID;
			mFromCToS.length=len;
			postOffice->Send(pFromCToS,mFromCToS,data);
		}
		return;
	}
			
	
	pFromCToS.to=rand()%SERVERS;											/*Setting the machine ID and the MailBox ID for the client and server*/
	mFromCToS.to=0;
	pFromCToS.from = myMachineID;
	mFromCToS.from=currentThread->myProcessID;
	mFromCToS.length=len;	
	...
	//else
		//printf("\n%s : SENT REQUEST(%d,%s,%d) TO SERVER(%d,%d) FROM CLIENT(%d,%d)",syscallType,packetToSend.syscall,packetToSend.name1,packetToSend.index1,pFromCToS.to,mFromCToS.to,myMachineID,currentThread->myProcessID);
}
...
int receive(char *syscallType){								/*Function that performs PostOffice->Receive, which receives the data from server and gives it to client*/
	...
	postOffice->Receive(currentThread->myProcessID,&pFromSToC,&mFromSToC,data);		/*Performing the "receive" function, that receives the data from server to client*/
	
	
	memcpy((void *)&packetToReceive,(void*)data,len);		/*Performing copy function into a packet structure*/
	//printf("\nReceived Packet from Server(%d,%d) - ID - %d",pFromSToC.from,mFromSToC.from,packetToReceive.value);
	if(!packetToReceive.status){
		tempID = packetToReceive.value;
		return -1;
	}
...
serverPacket receiveAckPacket(char *syscallType){			/*Function that performs PostOffice->Receive , whcih is mainly used for handling Monitor Variables*/
	...
	postOffice->Receive(currentThread->myProcessID,&pFromSToC,&mFromSToC,data); 		/*Performing the "receive" function, that receives the data from server to client*/
	memcpy((void *)&packetToReceive,(void*)data,len);		/*Performing copy function into a packet structure*/
	//printf("\nPACKET RECEIVED FROM SERVER(%d,%d), ID : %d",pFromSToC.from,mFromSToC.from,packetToReceive.value);
	return packetToReceive;
}

void EndSimulation_Syscall(int ID){
	if(ID==1){
		clientPacket packetToSend;					
		packetToSend.syscall=SC_EndSimulation;                                    
		send("ENDSIM",packetToSend);
	}	
	printf("\nAll the customers exited the store, ending SuperMarket Simulation\n");
	interrupt->Halt();
}
...
int CreateLock_Syscall(int vaddr, int len,int num) 
{
	int lockID;
	int localFlag;
	int newLen;
	...
	copyin(vaddr,newLen,buf);
	buf[newLen]='\0';
	
	#ifdef NETWORK
		
		sprintf(name,"%s%d",buf,num);
		name[len]='\0';
		clientPacket packetToSend;						/*Creating a packet, which initializes itself with the constructor*/
		strcpy(packetToSend.name1,name);			/*Copying the required data into the packet*/	
		...
		if(lockID==-1){									/*Halts the program if the index received is -1*/
			printf("\nCREATELOCK : ERROR CREATING LOCK%d",tempID);
			interrupt->Halt();
		}
		else{											/*The lock is created if the program comes to this else part*/
			printf("\nCREATELOCK : LOCK %d CREATED - NAME : %s",lockID,name);
		}
	#else	
...
void AcquireLock_Syscall(int lockID) 
{
	int localFlag;
		...
		if(lockID == -99)
			EndSimulation_Syscall(0);
		if(lockID==-1){                                         /*Halts the program if the index received is -1*/
			printf("\nLOCK ACQUIRE : ERROR ACQUIRING LOCK%d",tempID);       
			interrupt->Halt();                                     
		}                                                          
...
void ReleaseLock_Syscall(int lockID) 
{
	int localFlag;
		...
		packetToSend.index1 = lockID;                               
		packetToSend.value = SC_ReleaseLock;                             
		...
		if(lockID==-1){                                             /*Halts the program if the index received is -1*/
			printf("\nLOCK RELEASE : ERROR RELEASING LOCK%d",tempID);           
			interrupt->Halt();                                         
		}                                                              
...
void DestroyLock_Syscall(int lockID) 
{
	int localFlag;
		...
		if(lockID==-1){                                         /*Halts the program if the index received is -1*/  
			printf("\nDESTROYLOCK : ERROR DESTROYING LOCK%d",tempID);   
			interrupt->Halt();                                     
...
int CreateCV_Syscall(int vaddr, int len,int num) 
{
	int cvID;
	int localFlag;
	...
	buf[newLen]='\0';
		...
		name[len]='\0';
		clientPacket packetToSend;							/*Creating a packet, which initializes itself with the constructor*/
		strcpy(packetToSend.name1,name);                
		...
		if(cvID==-1){                                       /*Halts the program if the index received is -1*/     
			printf("\nCREATECV : ERROR CREATING CV%d",tempID);        
			interrupt->Halt();
		}
		else{											/*The CV is created if the program comes to this else part*/
			printf("\nCREATECV CV %d CREATED - NAME : %s",cvID,name);
		}
	#else
		...
		cvID = nextCVIndex++;
		if(localFlag==0)
			printf("\nCREATECV : CV1111 %d CREATED",cvID);
		KernelLock->Release();
	#endif
	return cvID;
...
void SignalCV_Syscall(int lockID, int cvID) {
	...
	int localFlag;
		...
		if(cvID == -1)                                      /*Halts the program if the index received is -1*/      
			{                                               
				printf("\nERROR OCCURED IN SIGNAL CV%d",tempID);      
				interrupt->Halt();                         
			}
		else												/*The CV is signalled if the program comes to this else part*/
			{
				//printf("\nCV%d SIGNAL IS SUCCESSFUL FOR PROCESS %d",cvID,currentThread->myProcessID);
				return;
			}
	#else
...
void WaitCV_Syscall(int lockID, int cvID) 
{
	int localFlag;
		...
		if(cvID == -1)                                  /*Halts the program if the index received is -1*/      
			{                                           
				printf("\nERROR OCCURED IN WAIT CV%d",tempID);    
				interrupt->Halt();                      
			}                                           
		else                                            /*The CV is put to wait if the program comes to this else part*/
			{
				//printf("\nCV%d  WAIT IS SUCCESSFUL FOR PROCESS %d",cvID,currentThread->myProcessID);
				return;
			}
	#else
...
void BroadcastCV_Syscall(int lockID, int cvID)
 {
	int localFlag;
		...
		if(cvID == -1)                                      /*Halts the program if the index received is -1*/      
			{                                               
				printf("\nERROR OCCURED IN BROADCAST CV%d",tempID);   
				interrupt->Halt();                          
			}                                               
...
int Exec_Syscall(int vaddr,int len) 
{
	...
	processCount++;
	processTable[spaceID].mailBoxID = mailBoxID++;
...
int CreateMV_Syscall(int vaddr,int len,int initialValue,int num){		/*System call for creating a monitor variable*/
	...
	strcpy(packetToSend.name1,name);						/*Assigning the required datea in the packet*/
	...
	printf("\nCREATEMV : MV %d CREATED, NAME : %s",mvID,name);					/*The MV is created if the program comes to this else part*/
...
int GetMV_Syscall(int mvID){									/*System call for getting a monitor variable*/
	...
	if(receivedPacket.status == true){
		//printf("\nGETMV : GOT MV%d, VALUE : %d",mvID,receivedPacket.value);
		return receivedPacket.value;
	}
	else{
		printf("\nGETMV : ERROR GETTING MV%d",mvID);
		interrupt->Halt();
		return -1;
	}
}
...
void ExceptionHandler(ExceptionType which) {
	...
	switch (type) {
		...
		case SC_EndSimulation:
			EndSimulation_Syscall(machine->ReadRegister(4));
			break;
